...
describe("tests client.treasury.inboundTransfer.list", () => {
  test.concurrent(
    "GET /v1/treasury/inbound_transfers | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      ...
      const [rawResponse, response] = await Promise.all([
        client.treasury.inboundTransfer
          ...
          .asResponse(),
        client.treasury.inboundTransfer.list({
          data: {},
...
describe("tests client.treasury.inboundTransfer.get", () => {
  test.concurrent(
    "GET /v1/treasury/inbound_transfers/{id} | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      const client = new Stripe({
        username: "API_USERNAME",
        password: "API_PASSWORD",
      ...
      const [rawResponse, response] = await Promise.all([
        client.treasury.inboundTransfer
          .get({ data: {}, id: "string" })
          .asResponse(),
        client.treasury.inboundTransfer.get({ data: {}, id: "string" }),
      ]);
      expect(rawResponse.status).toBe(200); // Exact status code match
      // Response body automatically validated by Zod schema during deserialization
      expect(response).toBeDefined();
...
describe("tests client.treasury.inboundTransfer.cance", () => {
  test.concurrent(
    "POST /v1/treasury/inbound_transfers/{inbound_transfer}/cancel | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      const client = new Stripe({
        username: "API_USERNAME",
        password: "API_PASSWORD",
