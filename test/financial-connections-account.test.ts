...
describe("tests client.financialConnections.account.list", () => {
  test.concurrent(
    "GET /v1/financial_connections/accounts | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      ...
      const [rawResponse, response] = await Promise.all([
        client.financialConnections.account.list({ data: {} }).asResponse(),
        client.financialConnections.account.list({ data: {} }),
      ]);
...
describe("tests client.financialConnections.account.get", () => {
  test.concurrent(
    "GET /v1/financial_connections/accounts/{account} | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      const client = new Stripe({
        username: "API_USERNAME",
        password: "API_PASSWORD",
      ...
      const [rawResponse, response] = await Promise.all([
        client.financialConnections.account
          .get({ data: {}, account: "string" })
          .asResponse(),
        client.financialConnections.account.get({
          data: {},
          account: "string",
        }),
      ]);
...
describe("tests client.financialConnections.account.disconnect", () => {
  test.concurrent(
    "POST /v1/financial_connections/accounts/{account}/disconnect | testId: success_default | Success test with response schema validation. Expects status code 200",
    async () => {
      const client = new Stripe({
        username: "API_USERNAME",
        password: "API_PASSWORD",
